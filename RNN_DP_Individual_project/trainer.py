...
class RNN_depth_trainer:
    ...
    def initDataloader(self,
                       dataset_dir,
                       batch_size=1,  # Fixed to 1 for computational reasons
                       img_height=128,
                       img_width=416,
                       num_views=10,
                       num_epochs=50,
    ...
    def construct_model(self, data_dict):
        ...
        for i in range(self.num_views):
            ...
            # Pose prediction
            pred_pose, hidden_state_pose = pose_net(tf.concat([image, pred_depth], axis=3), hidden_state_pose,
                                                    is_training=True)
    ...
    def compute_loss(self, estimates, data_dict, global_step):  #
        ...
        for i in range(self.num_views):
            ...
            Dx1, Dy1 = gradient(depth_slice, 1);
            gtDx1, gtDy1 = gradient(gt_depth_slice, 1)
            Dx2, Dy2 = gradient(depth_slice, 2);
            gtDx2, gtDy2 = gradient(gt_depth_slice, 2)
            Dx4, Dy4 = gradient(depth_slice, 4);
            gtDx4, gtDy4 = gradient(gt_depth_slice, 4)
            Dx8, Dy8 = gradient(depth_slice, 8);
            gtDx8, gtDy8 = gradient(gt_depth_slice, 8)
        ...
        for i in range(self.num_views):
            ...
            for j in range(i - 1, -1, -1):
                ...
                fw_threshold_flow = tf.where(tf.less_equal(0.05 * tf.norm(flow_fw, axis=3), 3.0),
                                             tf.ones_like(fw_flowdiff) * 3.0, 0.05 * tf.norm(flow_fw, axis=3))
                bw_threshold_flow = tf.where(tf.less_equal(0.05 * tf.norm(flow_bw, axis=3), 3.0),
                                             tf.ones_like(fw_flowdiff) * 3.0, 0.05 * tf.norm(flow_bw, axis=3))
                fw_occ_mask = tf.expand_dims(
                    tf.where(tf.less_equal(fw_flowdiff, fw_threshold_flow), tf.ones_like(fw_flowdiff),
                             tf.zeros_like(fw_flowdiff)), axis=-1)
                bw_occ_mask = tf.expand_dims(
                    tf.where(tf.less_equal(bw_flowdiff, bw_threshold_flow), tf.ones_like(fw_flowdiff),
                             tf.zeros_like(fw_flowdiff)), axis=-1)
        ...
        tf.summary.image('proj_img_fw', tf.concat([previous_scene_fw, proj_img_fw, image_slice_fw], axis=1),
                         max_outputs=1)
        tf.summary.image('proj_img_bw', tf.concat([image_slice_fw, proj_img_bw, previous_scene_fw], axis=1),
                         max_outputs=1)
    ...
    def tb_summary(self, output_dict, loss, all_losses):
        ...
        color_depth_fw = tf.expand_dims(
            tf.py_func(depth_plasma, [output_dict['depth_slice_fw'][0, :self.img_height - 10, :, 0]], tf.float32),
            axis=0)
        color_depth_bw = tf.expand_dims(
            tf.py_func(depth_plasma, [output_dict['depth_slice_bw'][0, :self.img_height - 10, :, 0]], tf.float32),
            axis=0)

        tf.summary.image('color_depth_fw10', color_depth_fw)
        tf.summary.image('color_depth_bw10', color_depth_bw)

        color_depth_fw1 = tf.expand_dims(
            tf.py_func(depth_plasma, [output_dict['depth_slice_fw1'][0, :self.img_height - 10, :, 0]], tf.float32),
            axis=0)
        color_depth_fw5 = tf.expand_dims(
            tf.py_func(depth_plasma, [output_dict['depth_slice_fw5'][0, :self.img_height - 10, :, 0]], tf.float32),
            axis=0)
